...
public class UsuarioDAO extends DAO<Usuario> {
		...
		try {
			...
			getStmt().executeUpdate(
					"DELETE FROM usuarios WHERE codigo_usuario ='"
							+ userAux.getCodigoUsuario() + "'");
			fecharBanco();
		} catch (Exception e) {
			throw new HumQueCaroException("- Erro ao remover Usuario -"
					+ e.getMessage());
		}
	...
	@Override
	public void altera(Usuario usuario) throws HumQueCaroException {
		try {
			abrirBanco();
			...
			getStmt().executeUpdate(
					"UPDATE usuarios SET codigo_usuario = '"
							...
							+ "' WHERE codigo_usuario like '" + usuario.getCodigoUsuario()
							+ "'");

			fecharBanco();

		} catch (SQLException exception) {
			throw new HumQueCaroException("Usuario nao foi alterado!"
	...
	public Usuario procura(String algo) throws HumQueCaroException {
		try {
			...
			rs = getStmt().executeQuery(
					"SELECT * FROM usuarios WHERE usuarios.email like '" + algo
							+ "'");
			while (rs.next()) {
				pessoa = new Usuario(rs.getString("email"),
